...
:::message alert
**このツールはClaude の使用量を消費します**

レート制限情報の取得のため、5分ごとにAnthropicのAPIへリクエストを送信します。詳細は後述の「[仕組みと注意点](#仕組みと注意点)」セクションを参照してください。
:::
...
### レート制限情報の取得（API呼び出しあり）

使用率（5時間・週間パーセント）を取得するため、このツールは **Anthropic API へ実際にリクエストを送信します**。

具体的には `https://api.anthropic.com/v1/messages` に対して、以下のような最小限のリクエストを発行します：
...
`max_tokens: 1`・内容は `.` の1文字というトークン消費を最小化した設計ですが、**このリクエスト自体がレート制限の消費としてカウントされます**。

:::message alert
**使用量への影響について**

- 1回のAPIリクエストにつき、入力トークン（システムプロンプト相当分 + 1文字）・出力1トークンが消費されます
- tmuxの更新間隔（デフォルト30秒）でスクリプトが呼ばれますが、**実際のAPI呼び出しは5分に1回**（キャッシュTTL: 300秒）に抑えられています
- つまり最大で **1時間あたり約12回** のAPIリクエストが発生します
- 消費量自体は微量ですが、「使用量を監視するツール自体が使用量を消費する」という点を理解したうえで使用してください
- すでに使用量が上限に近い状態では、このツール自体が上限到達を早める可能性があります
:::

レスポンスからは以下のヘッダーを読み取ります：
...
認証には `~/.claude/.credentials.json` に保存されたOAuthトークンを使用します（Claude Code のインストール時に生成されるファイルです）。
...
これにより30秒ごとに `claude-usage` が実行されます。スクリプト自体は30秒ごとに動作しますが、Anthropic APIへの実際のリクエストは5分に1回（キャッシュTTL: 300秒）に制限されています。コスト表示はローカルのJSONLファイルを読むだけなので、APIは呼び出しません。
