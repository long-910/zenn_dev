...
## Phase 5: Covering Tracks（痕跡消去）

| ツール / 手法 | 用途 | 特徴 |
|---|---|---|
| **Metasploit: clearev** | Meterpreter からイベントログ削除 | `clearev` コマンドで Windows ログをクリア |
| **Auditpol** | Windows 監査ポリシー操作 | ログ記録自体を無効化 |
| **Timestomp** | タイムスタンプ改ざん | アクセス日時・更新日時を偽装 |
| **Shred / Wipe** | ファイルの完全削除 | 上書きを繰り返して復元不能にする |
| **Log Zapper** | Linux syslog 操作 | 特定 IP の記録を削除 |
| **Steghide / OpenStego** | ステガノグラフィ | 画像・音声データにコードを隠蔽 |
